<template>
    <div class="min-h-screen bg-background-light dark:bg-background-dark py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Content -->
            <div class="bg-surface-light dark:bg-surface-dark rounded-3xl shadow-xl border border-border-light dark:border-border-dark overflow-hidden min-h-[400px]">
                
                <!-- Loading State -->
                <div v-if="pending" class="flex flex-col items-center justify-center h-64">
                    <span class="material-symbols-outlined text-4xl text-primary-500 animate-spin mb-4">progress_activity</span>
                    <p class="text-text-secondary-light dark:text-text-secondary-dark font-medium">Cargando estadísticas...</p>
                </div>
                ...
                <div v-else-if="currentData && currentData.length > 0" class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-gray-50 dark:bg-white/5 border-b border-border-light dark:border-border-dark">
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-bold text-text-secondary-light dark:text-text-secondary-dark uppercase tracking-wider">Pos</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-text-secondary-light dark:text-text-secondary-dark uppercase tracking-wider">Jugador</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-text-secondary-light dark:text-text-secondary-dark uppercase tracking-wider">Equipo</th>
                                <th class="px-6 py-4 text-right text-xs font-bold text-text-secondary-light dark:text-text-secondary-dark uppercase tracking-wider">
                            ...
                            <tr v-for="(item, index) in currentData" :key="index" 
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center overflow-hidden mr-3">
                                            <img v-if="item.player_photo" :src="item.player_photo" :alt="item.player_name" class="h-full w-full object-cover">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-text-secondary-light dark:text-text-secondary-dark font-medium">
                                        {{ item.team_name }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div v-if="currentTab === 'cards'" class="flex items-center justify-end gap-3">
                                        <span class="flex items-center gap-1 px-2 py-1 rounded bg-yellow-100 text-yellow-800 text-xs font-bold">
                                            <span class="w-2 h-3 bg-yellow-500 rounded-sm inline-block"></span>
                                            {{ item.yellow_cards }}
                                        </span>
                                        <span class="flex items-center gap-1 px-2 py-1 rounded bg-red-100 text-red-800 text-xs font-bold">
                ...
                <div v-else class="flex flex-col items-center justify-center h-96 text-center p-8">
                    ...
                    <p class="text-text-secondary-light dark:text-text-secondary-dark max-w-md mx-auto">
                        Aún no se han registrado estadísticas para esta categoría. ¡Comienza a jugar partidos para ver los resultados aquí!
                    </p>
                </div>
            </div>
        </div>
...
import { useNuxtApp, useAsyncData, definePageMeta } from '#app'
