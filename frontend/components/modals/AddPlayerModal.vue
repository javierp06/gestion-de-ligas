            ...
            <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">
                        {{ $t('modals.add_player.name_label') }} <span class="text-red-500">*</span>
                    </label>
                    <input v-model="formData.name" type="text" required
                        ...
                        :placeholder="$t('modals.add_player.name_placeholder')" />
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">
                            {{ $t('modals.add_player.number_label') }}
                        </label>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">
                            {{ $t('modals.add_player.position_label') }}
                        </label>
                        <select v-model="formData.position"
                            ...
                            <option value="">{{ $t('modals.add_player.select_position') }}</option>
                            <option value="Portero">{{ $t('common.positions.goalkeeper') }}</option>
                            <option value="Defensa">{{ $t('common.positions.defender') }}</option>
                            <option value="Mediocampista">{{ $t('common.positions.midfielder') }}</option>
                            <option value="Delantero">{{ $t('common.positions.forward') }}</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">
                        {{ $t('modals.add_player.birth_date_label') }}
                    </label>
                ...
                <div class="pt-4 flex gap-3">
                    ...
                    <button type="submit" :disabled="loading"
                        ...
                        {{ loading ? $t('modals.add_player.saving') : $t('modals.add_player.submit') }}
                    </button>
                </div>
            </form>
        </div>
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            toastStore.success(t('modals.add_player.success'))
            emit('created')
            emit('close')
        }
    } catch (error: any) {
        toastStore.error(error.response?.data?.message || t('modals.add_player.error'))
    } finally {
        loading.value = false
    }
}
