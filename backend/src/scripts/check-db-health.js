  ...
  try {
    ...
    if (columns.length > 0) {
      ...
      if (roleType.includes("'user'") && roleType.includes("'organizer'") && roleType.includes("'admin'")) {
        ...
        console.log('   ⚠️  ENUM no coincide con lo esperado');
      }
    }
    console.log('');
    ...
    if (totalSports === 0) {
      console.log('   ⚠️  No hay deportes registrados');
    ...
    for (const check of checks) {
      try {
        ...
        console.log(`   ⚠️  ${check.table}.${check.fk} → ${check.ref}.id (no se pudo verificar)`);
      }
    }
